...
static pid_t do_newthread(process_t *parent, int function);
...
do_fork(process_t *parent)
{
    ...
    pid_t process_id = current->p_pid;
    process_t *child;
    int i;
    for (i=0; ;i++) {
        if (i >= 15) return -1;
        process_id = (process_id + 1) % NPROCS;
        if ((process_id != 0) && proc_array[process_id].p_state == P_EMPTY) {
            child = &proc_array[process_id];
        }
    }
